...
import com.sky.constant.MessageTypeConstant;
...
public class OrderServiceImpl implements OrderService {
    ...
    public void paySuccess(String outTradeNo) {
        ...
        Map map = new HashMap();
        map.put("type", MessageTypeConstant.ORDER_COME);
    ...
    @Override
    @Transactional
    public PageResult listHistoryOrders(int pageNum, int pageSize, Integer status) {
        // 设置分页
    ...
    private List<OrderVO> getOrderVOList(Page<Orders> page) {
        ...
        if (!CollectionUtils.isEmpty(ordersList)) {
            for (Orders orders : ordersList) {
                ...
                // 将订单菜品信息封装到 orderVO 中，并添加到 orderVOList 中
...
//        }
        ...
        // 管理端取消订单需要退款，根据订单 id 更新订单咋混个太、取消原因、取消时间
        Orders orders = Orders.builder()
    ...
    public void delivery(Long id) {
        ...
        Orders orders = Orders.builder()
                .id(id)
                .status(Orders.DELIVERY_IN_PROGRESS) // 更新状态为 4（派送中）
                .build();
        orderMapper.update(orders);
    }
