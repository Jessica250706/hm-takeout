...
import {
  ...
  deleteDish,
  dishStatusByStatus,
  // ⚠️ 导入的 API 与 state 变量同名，用 as 重命名
...
// ---- 方法 ----
const init = async (isSearchParam?: boolean | Event) => {
...
const getDishCategoryList = () => {
  dishCategoryListApi({ type: 1 })
    .then((res: any) => {
      if (res && res.data && res.data.code === 1) {
        dishCategoryList.value = (res.data?.data ?? []).map((item: any) => ({
          value: item.id,
          label: item.name,
        }))
      }
