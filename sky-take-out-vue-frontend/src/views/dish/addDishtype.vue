...
const selectHandle = (val: any, key: any) => {
...
const init = async () => {
  queryDishById(route.query.id).then((res: any) => {
...
// 口味位置记录
const flavorPosition = (indexVal: number) => {
  index.value = indexVal
}

// 添加口味标签
const keyDownHandle = (val: any) => {
  // 去掉对全局 event 的使用，改为在事件回调里自行处理
  if (val?.target) {
...
const submitForm = (_formName: string, st?: string) => {
  ruleFormRef.value?.validate((valid: boolean) => {
    ...
    params.flavors = dishFlavors.value.map((obj: any) => ({
      ...obj,
