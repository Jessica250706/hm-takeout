  ...
  <div class="dashboard-container">
    ...
    <div class="container" :class="{ hContainer: tableData.length }">
      ...
      <el-table v-if="tableData.length" :data="tableData" stripe class="tableBox">
        ...
        >
          <template #default="{ row }">
            <span>￥{{ (Number(row.amount).toFixed(2) * 100) / 100 }}</span>
          </template>
        </el-table-column>
        <el-table-column
...
const dishSubtotal = computed(() => {
  const a = Number(diaForm.value.amount) || 0
  const p = Number(diaForm.value.packAmount) || 0
  return (a - 6 - p).toFixed(2)
})

const dishSubtotalRaw = computed(() => {
  const a = Number(diaForm.value.amount) || 0
  const p = Number(diaForm.value.packAmount) || 0
  const v = Number((a - 6 - p).toFixed(2))
  return ((v * 100) / 100).toFixed(2)
})

const packAmountDisplay = computed(() => {
  const p = Number(diaForm.value.packAmount)
  return p ? ((p * 100) / 100).toFixed(2) : ''
})

const totalAmountDisplay = computed(() => {
  const a = Number(diaForm.value.amount)
  return a ? ((a * 100) / 100).toFixed(2) : ''
})

// ---- 方法 ----
const getOrderType = (rowData: any) => {
...
const init = (activeIndex: number = 0, isSearchParam?: boolean) => {
  isSearch.value = isSearchParam === true
  const params = {
    page: page.value,
    ...
    .then((res: any) => {
      if (res.data.code === 1) {
        ...
        if (
          ...
          goDetail(first.id, first.status, first)
        }
      } else {
        ElMessage.error(res.data.msg)
      }
...
const goDetail = async (id: any, status: number, rowData?: any) => {
  ...
  row.value = rowData || { id: route.query.orderId, status }
...
// 派送、完成
const cancelOrDeliveryOrComplete = (statusVal: number, id: string) => {
  const params = { status: statusVal, id }
  const api = statusVal === 3 ? deliveryOrder : completeOrder
  api(params)
    .then((res: any) => {
      if (res.data.code === 1) {
        ElMessage.success('操作成功')
