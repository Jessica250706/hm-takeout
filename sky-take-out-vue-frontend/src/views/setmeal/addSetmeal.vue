      ...
      >
        <div>
          ...
          <el-form-item label="套餐分类:" prop="idType">
            <el-select v-model="ruleForm.idType" placeholder="请选择套餐分类">
              <el-option
        ...
        <div>
          <el-form-item label="套餐图片:" required prop="image">
            <ImageUpload :prop-image-url="imageUrl" @imageChange="imageChange">
              图片大小不超过2M
              <br />
        ...
        <div class="subBox address">
          <el-form-item>
            ...
            <el-button v-if="actionType == 'add'" type="primary" @click="submitForm(true)">
              保存并继续添加
            </el-button>
          </el-form-item>
        </div>
...
const rules = computed(() => ({
  name: {
    required: true,
    validator: (_rule: any, val: string, callback: (err?: Error) => void) => {
      if (!val) {
  ...
  price: {
    required: true,
    validator: (_rules: any, val: string, callback: (err?: Error) => void) => {
...
const getDishTypeList = () => {
  getCategoryList({ type: 2, page: 1, pageSize: 1000 }).then((res: any) => {
    if (res && res.data && res.data.code === 1) {
...
const submitForm = (st?: boolean) => {
  ruleFormRef.value?.validate((valid: boolean) => {
    ...
    params.setmealDishes = dishTable.value.map((obj: any) => ({
      copies: obj.copies,
      dishId: obj.dishId,
      name: obj.name,
      price: obj.price,
    ...
    params.categoryId = ruleForm.idType
